...
for1 <- rforecast(s, groups = list(1, c(1,4), 1:4, 1:6),len = 12)
matplot(data.frame(for1), 
        type = "b", pch = c("1", "2", "3", "4"), ylab = "")

# Vector forecast, the forecasted points are added to the base series
for1a <- vforecast(s,groups = list(1, trend = c(1,4), 1:4, 1:6),
...
plot(for3, include = 36, shadecols = "green", type = "l", main = "Confidence intervals")
set.seed(3)
